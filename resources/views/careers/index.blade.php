    ...
    <div class="mx-auto max-w-7xl">
        <div class="mb-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p class="max-w-xl text-sm leading-6 text-gray-600">
                {{ $showAll ? 'Pilih posisi yang sesuai dengan minat dan pengalaman Anda.' : 'Kami tampilkan 3 lowongan terbaru. Klik tombol di bawah untuk melihat semua lowongan yang tersedia.' }}
            </p>
        </div>

        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
            @forelse($vacancies as $vacancy)
                <article class="flex min-h-[260px] flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition hover:border-blue-200 hover:shadow-md">
                    <h2 class="line-clamp-2 text-lg font-extrabold leading-snug text-gray-950">
                        {{ $vacancy->title }}
                    </h2>

                    <div class="mt-5 space-y-3 text-sm text-gray-700">
                        <div class="flex items-center gap-3">
                            <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path d="M10 6V5a2 2 0 0 1 2-2h0a2 2 0 0 1 2 2v1M4 8h16v11H4z" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </span>
                            <span class="line-clamp-1">{{ $vacancy->employment_type ?: 'Staff' }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <span class="line-clamp-1">{{ $vacancy->deadline ? $vacancy->deadline->translatedFormat('l, d M Y') : 'Dibuka sampai terpenuhi' }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600">
                            ...
                            <span class="line-clamp-1">{{ $vacancy->location ?: 'Fleksibel' }}</span>
                        </div>
                    </div>

                    <div class="mt-auto flex justify-end pt-5">
                        <a href="{{ route('careers.show', $vacancy->id) }}"
                            class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-blue-700">
                            Details
                        </a>
                    </div>
                </article>
        ...
        @elseif($totalVacancies > 3)
            <div class="mt-10 flex justify-center">
                <a href="{{ route('careers.index', ['show' => 1]) }}"
                    class="inline-flex min-h-12 items-center justify-center rounded-xl bg-blue-600 px-7 text-sm font-black text-white transition hover:bg-blue-700">
